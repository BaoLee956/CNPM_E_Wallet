/**
 * frontend/src/components/withdraw/WithdrawForm.tsx
 *
 * Form rút tiền với hỗ trợ chọn ngân hàng đã liên kết (gateway flow).
 * Tự động chọn tài khoản mặc định khi load.
 */
...
export function WithdrawForm({
  ...
  // Load danh sách ngân hàng liên kết
  useEffect(() => {
    bankService
      .getLinkedBanks()
      .then((banks) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: WithdrawPayload = {
      ...
      ...(selectedBankId ? { linkedBankId: selectedBankId } : {}),
    };
        ...
        {selectedBank && bankInfo && amount && parseFloat(amount) >= 10_000 && (
          <div className="rounded-xl bg-surface-sunken p-3 space-y-1.5 text-sm">
            <div className="flex justify-between">
              <span className="text-secondary">Đến tài khoản</span>
              <span className="font-medium">
                {bankInfo.name} *{selectedBank.accountNumber.slice(-4)}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-secondary">Số tiền rút</span>
