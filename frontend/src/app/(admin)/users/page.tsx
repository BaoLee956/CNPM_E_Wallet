          ...
          <div className="mb-4">
            ...
            <div className="space-y-2">
              ...
              <div className="bg-slate-800/60 rounded-xl p-3">
                ...
                <p className="text-sm text-slate-200">{user.phoneNumber}</p>
              </div>
              <div className="bg-slate-800/60 rounded-xl p-3">
                <p className="text-[10px] text-slate-500 uppercase font-semibold tracking-wider mb-0.5">User ID</p>
                <p className="text-sm text-slate-200 font-mono">{user.userId}</p>
              </div>
            </div>
          </div>

          {user.status === "Suspicious" && (
            <div className="mt-4 p-4 rounded-2xl border border-amber-500/30 bg-amber-500/5">
              <div className="flex items-center gap-2 mb-2">
                <ShieldAlert size={16} className="text-amber-400" />
                <span className="text-xs font-bold text-amber-400 uppercase tracking-wider">Risk Assessment</span>
              </div>
              <p className="text-xs text-amber-300/80 leading-relaxed">
                This account has been flagged by the fraud detection system. Recent login from unexpected IP and rapid consecutive transactions detected. Manual review is recommended.
              </p>
            </div>
          )}
        </div>
...
export default function UsersPage() {
  ...
  const { lockUser, unlockUser, fetchUsers } = useAdminUsers();
  ...
  const handleToggleConfirm = async () => {
    ...
    const { userId, action, reason } = pendingToggle;
    if (action === "lock") {
      await lockUser(userId, reason || "Locked by admin");
    } else {
      await unlockUser(userId, reason || "Unlocked by admin");
    }
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {errorUsers && (
        <div className="flex items-center gap-3 bg-rose-500/10 border border-rose-500/30 rounded-2xl px-5 py-4">
          ...
          <button
            onClick={() => fetchUsers({ page: 1, limit: 20 })}
              ...
              {isLoadingUsers ? (
                ...
                <tr>
                  <td colSpan={6} className="px-5 py-16 text-center text-slate-500 text-sm">
                    {errorUsers ? "Failed to load users." : "No users match your filters."}
                  </td>
                </tr>
              ) : (
                filtered.map((user) => (
